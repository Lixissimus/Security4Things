...
In the last decades, the rise embedded systems changed daily lives. % Source?
...
This would mean that either there is a small set of assigned keys, which would lead to vulnerabilities to brute-force attacks; or each mote gets an individual key, which requires a secure transmission and attribution of keys to motes\footnote{This would also increase cost of motes, but they need to be as cheap as possible in order to be be affordable in great numbers.}.
Anyway, pre-initialization requires somebody other than the user/installer to know the keys, which diminishes security.

This paper proposes an alternative way to transmit initial keying material via light.
...
Moreover, multiple devices could be programmed concurrently by just placing them next to each other.

We implemented prototype for an Android smart phone and a Contiki\footnote{http://www.contiki-os.org/} mote.
Contiki is an open source, state-of-the-art operating system for IoT devices.
For Android phones, we implemented two versions of a light emitting application.
...
Therefore, the app has to guess, when the mote has successfully done the calibration and synchronization.
This can either be done by using a fixed time (let's say the app stays in calibration/synchronization phase for 10 seconds), or by user interaction (the user presses a button, when the mote indicates that it's ready).
...
Once the init pattern has been detected, the mote imediately switches to the data sending phase.

\subsection{Data sending}
\label{sub:data_sending}

In the data sending phase, we can finally transmit the data packet.
As we can see in Figure~\ref{fig:data_packet}, the packet contains the data length, the actual data and a checksum used for validation of the transmission in the validation phase~\ref{sub:validation}.
...
- two versions with different strengths
- first web app deployed with cordova as native app
- functionality: almost all parameters were adjustable, i.e. length of one bit, length of initialization phase, initialization-pattern, start-pattern, if hamming-encoding should be used.
- biggest advantage: runs also in browser an easily portable to iPhone/windows phone/...
- but also fast prototyping
- downsides: javascript timer inaccuracy
- Flash light reacted very slowly
- white and dark screen background only possible way to go

-figure: both screenshots next to each other

- second version: native android app
- still no real time possible (source!), but smaller stack (no phoneGap runtime and javascript interpreter required)
- native readjusting timer: if one event fires too late, delay won't add up
- flash light works as good as background flickering
- clearer design because of less controllable parameters: just bit-length and switches to use flashlight and hamming-encoding. (better for end users)
...
Including the light app on a mote automatically redefines \textit{NETSTACK\_CONF\_LLSEC}\footnote{This global constant defines the link layer driver that is used by the mote.} to point to our link layer driver wrapper (and no further configuration needs to be done?).
...
This would also make it easier to initalize several motes at once, which because of lack of synchronization is currently quite tedious.
